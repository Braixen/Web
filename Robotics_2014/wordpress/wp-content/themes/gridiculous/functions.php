...
function gridiculous_excerpt_more( $more ) {
	...
	return '&hellip;<p class="more-link-p"><a class="more-link" href="' . get_permalink( $post->ID ) . '">Read more &rarr;</a></p>';
}
...
function gridiculous_customize_register( $wp_customize ) {
	...
	if ( $wp_customize->is_preview() && ! is_admin() )
		add_action( 'wp_footer', 'gridiculous_customize_preview', 21);
}
 ...
 * @since Gridiculous 1.0.0
 */
function gridiculous_customize_preview() {
	?>
<script type="text/javascript">
( function( $ ){
	wp.customize( 'gridiculous_theme_options[width]', function( value ) {
		value.bind( function( to ) {
			$( '#page' ).removeClass().addClass( 'grid ' + to );
		} );
	} );

	wp.customize( 'gridiculous_theme_options[primary]', function( value ) {
		value.bind( function( to ) {
			var secondary = 12 - to.replace( 'c', '' ),
				centered = ( $( '#primary' ).hasClass( 'centered' ) ) ? ' centered' : '';

			$( '#primary' ).removeClass().addClass( to + centered );
			$( '#secondary' ).removeClass().addClass( 'c' + secondary );
		} );
	} );
} )( jQuery )
</script>
...
function gridiculous_add_admin_bar_menu_item() {
    ...
    if ( current_user_can( 'edit_theme_options' ) && is_admin_bar_showing() )
    	$wp_admin_bar->add_menu( array( 'id' => 'customize_gridiculous', 'title' => sprintf( __( 'Customize %s', 'gridiculous' ), 'Gridiculous' ), 'href' => admin_url( 'customize.php' ) ) );
}