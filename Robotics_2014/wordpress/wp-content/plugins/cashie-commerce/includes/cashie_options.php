...
if(!class_exists('cashie_options'))
{
  ...
  class cashie_options
  {
		...
		function get_cashie_options()
    {
			...
			if ($this->post_vars['update_hash'])
			{
				$this->update_hash();
			}
			
			// Recreate pages if we have updated the hash or manually want to recreate pages
			if ($this->post_vars['update_hash'] || $this->post_vars['create_pages'])
			{
				$this->create_pages();
				$this->details_dynamic();
			}
			
			if ($this->post_vars['update_details_dynamic'])
			{
				$this->details_dynamic();
			}
			
			// Add/delete product detail pages
			if (isset($this->post_vars['mode']))
			{
	...
	function create_pages()
  {
		...
		$auto_post = array(
			...
			$auto_post = array(
				...
				'post_author' => $current_user->ID,
				'post_category' => '',
				'post_content' => '<script type="text/javascript">document.write(unescape("%3Cscript src=\'" + (\'https:\' == document.location.protocol ? \'https://\' : \'http://\') + "'.$this->cashie_s3.'.s3.amazonaws.com/js/response-success.js\' type=\'text/javascript\'%3E%3C/script%3E"));</script>',
				'post_date' => '',
				'post_date_gmt' => '',
				'post_excerpt' => '',
				'post_name' => '',
			...
			$auto_post = array(
				...
				'post_author' => $current_user->ID,
				'post_category' => '',
				'post_content' => '<script type="text/javascript">document.write(unescape("%3Cscript src=\'" + (\'https:\' == document.location.protocol ? \'https://\' : \'http://\') + "'.$this->cashie_s3.'.s3.amazonaws.com/js/response-failure.js\' type=\'text/javascript\'%3E%3C/script%3E"));</script>',
				'post_date' => '',
				'post_date_gmt' => '',
				'post_excerpt' => '',
				'post_name' => '',
	...
	function details_dynamic()
  {
		...
		if (isset($this->post_vars['details_dynamic']))
			$this->settings['details_dynamic'] = intval($this->post_vars['details_dynamic']);
			
		if ($this->settings['details_dynamic']==1)
		{
			$auto_post = array(
		...
		{
			if (isset($this->settings['url_details_dynamic']))
			{
  ...
  function detail_page($mode=null, $product_id=null, $prodname=null)
  {
		global $current_user;
		$id = '';
		if (isset($this->settings['detail_pages'][$product_id]))
		{
